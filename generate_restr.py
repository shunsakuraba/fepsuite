...
def generate_itp(args):
    with open(args.restrinfo) as fh, open(args.itp, "w") as ofh:
        ...
        print("%d %d %d 1 %.3f %.3f" % (anchor_atoms[1], anchor_atoms[2], anchor_atoms[3], avgs[1] / math.pi * 180.0, args.angle_spring), file=ofh)
        print("%d %d %d 1 %.3f %.3f" % (anchor_atoms[2], anchor_atoms[3], anchor_atoms[4], avgs[2] / math.pi * 180.0, args.angle_spring), file=ofh)
        print(file=ofh)
        print("[ dihedrals ]", file=ofh)
        # uses functype=2 for restraining
        print("%d %d %d %d 2 %.3f %.3f" % (anchor_atoms[0], anchor_atoms[1], anchor_atoms[2], anchor_atoms[3], avgs[3] / math.pi * 180.0, args.dihedral_spring), file=ofh)
        print("%d %d %d %d 2 %.3f %.3f" % (anchor_atoms[1], anchor_atoms[2], anchor_atoms[3], anchor_atoms[4], avgs[4] / math.pi * 180.0, args.dihedral_spring), file=ofh)
        print("%d %d %d %d 2 %.3f %.3f" % (anchor_atoms[2], anchor_atoms[3], anchor_atoms[4], anchor_atoms[5], avgs[5] / math.pi * 180.0, args.dihedral_spring), file=ofh)
        print(file=ofh)
